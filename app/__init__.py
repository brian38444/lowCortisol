...
def disp_homepage():
    ...
    total_vtubers = cursor.execute("SELECT COUNT(*) FROM vtubers").fetchone()[0] or 0
    total_agencies = cursor.execute(
        "SELECT COUNT(DISTINCT agency) FROM vtubers WHERE agency IS NOT NULL AND agency != ''"
    ).fetchone()[0] or 0
    total_subs = cursor.execute(
        "SELECT COALESCE(SUM(total_subscriber_count), 0) FROM vtubers"
    ).fetchone()[0] or 0
    ...
    trending = cursor.execute("""
        ...
        ORDER BY total_subscriber_count DESC
        LIMIT 10
    """).fetchall()
